...
public sealed class RegisterAccountHandler : ICommandHandler<RegisterAccountCommand, AuthResponse>
{
    ...
    public async Task<Result<AuthResponse>> HandleAsync(RegisterAccountCommand command, CancellationToken cancellationToken)
    {
        ...
        if (string.IsNullOrWhiteSpace(command.Password) || command.Password.Length < 8) return Result<AuthResponse>.Failure("Password must be at least 8 characters.");
    ...
    private static bool IsValidEmail(string email) => Regex.IsMatch(email ?? string.Empty, "^[^@\\s]+@[^@\\s]+\\.[^@\\s]+$");
}
